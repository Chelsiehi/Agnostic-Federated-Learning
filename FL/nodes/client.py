...
class CreateDataset(Dataset):
    """
    This class is used to create a custom dataset wrapped around PyTorch's Dataset class.
    It takes a dataset and a list of indices as input, allowing you to create a dataset containing specific samples from the original dataset.
    It implements methods like __len__ and __getitem__ required for dataset handling.
    """
...
# This is a base class for local FL clients
class LocalBase():
...
"""
This class inherits from LocalBase and represents a local client in the Federated Averaging (FedAvg) FL setting.
It includes a localround method for a local training round. It first validates the local model (local_validate)  and then updates weights (update_weights). 
It returns clients_params, which contain updated weights.
"""
...
"""
Similar to Fedavg_Local, this class inherits from LocalBase and represents a local client in the Adaptive Federated Learning (AFL) setting.
It includes a localround method for a local training round. It validates the local model and updates weights, but it also calculates the AFL loss.
"""
...
"""
This function creates an instance of a local FL client based on the specified FL type (FedAvg or AFL).
It takes args (configuration settings), train_dataset, val_dataset, and client_id as input and returns an instance of the corresponding local client class.
"""
